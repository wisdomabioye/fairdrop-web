...
export class LineraClientManager implements ILineraClientManager {
  ...
  async initializeReadOnly(): Promise<void> {
    ...
    if (this.mode !== ClientMode.UNINITIALIZED) {
      console.log('[ClientManager] Already initialized');
      return;
    }

    try {
      console.log('[ClientManager] Initializing read-only mode...');
      ...
      if (!this.faucet) {
        console.log('[ClientManager] Failed to instantiate Faucet');
      ...
      // Create temporary signer for guest mode
      ...
      console.log('[ClientManager] Claiming temporary chain...');
      this.chainId = await this.faucet.claimChain(this.wallet, tempOwner);
      console.log('[ClientManager] Temporary chain claimed:', this.chainId);
      ...
      console.log('[ClientManager] Read-only mode initialized successfully');
    } catch (error) {
  ...
  async connectWallet(metamaskSigner: SignerWithAddress): Promise<void> {
    ...
    try {
      console.log('[ClientManager] Connecting wallet...');
      ...
      if (needsNewWallet) {
        ...
        console.log('[ClientManager] Chain claimed:', this.chainId);
      }
      ...
      console.log('[ClientManager] Wallet connected successfully');
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      this.notifyStateChange({ error: err });
  ...
  async disconnectWallet(): Promise<void> {
    if (this.mode !== ClientMode.FULL) {
      console.log('[ClientManager] No wallet to disconnect');
      return;
    }

    console.log('[ClientManager] Disconnecting wallet...');
    ...
    console.log('[ClientManager] Wallet disconnected, reverted to read-only mode');
  }
  ...
  async switchWallet(newSigner: SignerWithAddress): Promise<void> {
    console.log('[ClientManager] Switching wallet...');
  ...
  async getApplication(appId: string): Promise<ApplicationClient | null> {
    if (!this.client) {
      console.warn('[ClientManager] Client not initialized');
      return null;
    }

    try {
      const app = await this.client.frontend().application(appId);
      return new ApplicationClientImpl(appId, app, this.mode === ClientMode.FULL);
    } catch (error) {
      console.error('[ClientManager] Failed to get application:', error);
      return null;
    }
  }
  ...
  async destroy(): Promise<void> {
    console.log('[ClientManager] Destroying client...');
  ...
  private notifyStateChange(additionalState?: Partial<ClientState>): void {
    ...
    this.stateListeners.forEach(listener => {
      try {
        ...
        console.error('[ClientManager] State listener error:', error);
      }
    });
  }
}
...
export function createLineraClient(config: ClientConfig): LineraClientManager {
  if (clientManagerInstance) {
    console.log('[ClientManager] Returning existing instance');
