...
import { MetaMaskSigner, isMetaMaskInstalled } from '@/lib/metamask/metamask-signer';
...
export function useWalletConnection(): UseWalletConnectionReturn {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      console.log('[useWalletConnection] Wallet connected successfully');
    } catch (err) {
  ...
  const disconnect = useCallback(async () => {
    ...
    try {
      ...
      console.log('[useWalletConnection] Wallet disconnected');
    } catch (err) {
      const error = err instanceof Error ? err : new Error(String(err));
  ...
  useEffect(() => {
    ...
    const handleAccountsChanged = async (accounts: unknown) => {
      ...
      if (accountList.length === 0) {
        ...
        console.log('[useWalletConnection] MetaMask accounts disconnected');
        await disconnect();
      } else if (isConnected) {
        // Account switched
        console.log('[useWalletConnection] MetaMask account changed, switching wallet...');
        try {
          const newSigner = new MetaMaskSigner();
          await clientManager.switchWallet(newSigner);
        } catch (err) {
          console.error('[useWalletConnection] Failed to switch wallet:', err);
